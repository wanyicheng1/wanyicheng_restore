...
{
    QString name,number,Class,time,GPA,password;
    ...
    if(ui->radioButton_add->isChecked())
    {
        ...
        else
        {
            ...
            sql2="insert into student values ('"+name+"','"+number+"','"+Class+"','"+time+"','"+GPA+"','"+password+"')";
    ...
    else if(ui->radioButton_change->isChecked())
    {
        ...
        else
        {
            QString sql2;
            QSqlQueryModel *model2=new QSqlQueryModel;
            sql2="update student set number ='"+number+"',class='"+Class+"',EnterTime='"+time+"',GPA='"+GPA+"',password='"+password+"' where name='"+name+"'";
            model2->setQuery(sql2);
